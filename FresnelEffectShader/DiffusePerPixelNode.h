...
public:
    ...
    void draw(SceneNode *camera, float reflectance, float power){
        
        glUseProgram(programId);
        
		//fresnel reflectance ve fresnel power'i shader'a gonder
		GLint refLoc = glGetUniformLocation(programId, "reflectance");
		GLint powLoc = glGetUniformLocation(programId, "power");
		glUniform1f(refLoc, reflectance);
		glUniform1f(powLoc, power);
